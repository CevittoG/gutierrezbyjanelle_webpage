...
license    = (Σ design_labor of lines with line.digitalLicense) × digitalLicensePtg%   # opted in PER LINE
...
| `app/quote-calc/_components/QuoteCalculator.tsx` | Main UI on a single `lines: QuoteLine[]` state: "Add a package" grid + "Add an individual item" picker both append lines to one "On this quote" list; each line carries its own **digital file license** toggle (`QuoteLine.digitalLicense`) alongside qty/item/sale-type; other controls regrouped into **Project services** (rush, revisions), **Discounts** (vendor, family & friends, custom), and **Materials** (full color, custom paper). Misc add-ons kept. Sheet sync on mount/save; pulls live `catalog` + `assumptions` from `/api/config` |
...
`DraftConfig` holds the full quote state per draft as a single `lines: QuoteLine[]` array (each line `kind: "package"` with a `pkg`, or `kind: "item"` with an `itemKey`; both carry `qty`, a `digital` flag, and a `digitalLicense` flag — the license fee is opted into **per line**, not per quote, so a physically-printed line and a digitally-delivered line on the same quote can be charged differently), plus `miscAddOns: MiscAddOn[]` for free-form selling-price items, the quote-level service toggles (`rushFee`, `extraRevisions`), and the quote-wide discounts (`vendorIncentive`, `familyFriendsPtg`, `customDiscountPtg`). `DraftClientInfo` carries two notes: `notes` (private/hidden) and `clientNotes` (client-facing). `Draft.schemaVersion` is currently `4`; v1/v2/v3 drafts are auto-migrated on load into the unified `lines` model (see the drafts.ts row above). New fields need no version bump — `migrateConfig`/`migrateDraft` backfill them by spreading `DEFAULT_CONFIG` / `EMPTY_CLIENT_INFO`. A pre-existing quote-level `digitalLicense: true` on a legacy draft is backfilled onto every migrated line (reproducing the old whole-quote total exactly) rather than being kept as a `DraftConfig` field.
